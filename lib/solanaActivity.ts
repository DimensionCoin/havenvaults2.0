...
function extractTokenTransfers(tx: Record<string, unknown>): TokenTransferRecord[] {
 ...
 * - swap bought token (largest inbound non-USDC token)
 */
function normalizeToActivity(
  ...
  // Swap detection: if outflow USDC + inflow non-USDC token, call it swap
  ...
  if (kind === "swap") {
    // For swaps we consider "sold USDC" = USDC outflow from owner (not the delta)
    // This avoids weird cases where USDC out + USDC in exists.
