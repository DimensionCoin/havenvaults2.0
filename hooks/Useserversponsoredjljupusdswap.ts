// hooks/useServerSponsoredJLJupUSDSwap.ts
"use client";
...
export type JLJupUSDSwapStatus =
  | "idle"
  | "building"
  | "signing"
  | "sending"
...
/**
 * Swap JLJupUSD vault shares for another token (e.g., SOL, BTC, ETH)
 * This allows users to swap directly from their Plus savings without withdrawing first.
 */
export type JLJupUSDSwapInput = {
  fromOwnerBase58: string;
  /** The token to receive (e.g., SOL mint, BTC mint, etc.) */
  outputMint: string;
  /** Amount of JLJupUSD to swap (in UI units, e.g., 100.5) */
  amountUi: number;
  /** Optional slippage in basis points (default 50 = 0.5%) */
  slippageBps?: number;
  /** If true, swap entire JLJupUSD balance */
  isMax?: boolean;
};

export type JLJupUSDSwapResult = {
  signature: string;
  inputMint: string;
  outputMint: string;
  amountUnits: number;
...
export function useServerSponsoredJLJupUSDSwap() {
  ...
  const [status, setStatus] = useState<JLJupUSDSwapStatus>("idle");
  const [error, setError] = useState<JLJupUSDSwapError | null>(null);
