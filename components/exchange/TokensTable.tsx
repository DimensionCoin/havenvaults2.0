...
type TokensTableProps = {
  rows: Token[]; // ✅ already filtered + sorted + paged in parent
...
const TokensTable: React.FC<TokensTableProps> = ({
  ...
  return (
    <>
      ...
      <div className="mb-3 flex flex-col gap-2 text-xs text-zinc-500 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2">
          <span>
            {total > 0 ? (
              <>
                Showing <span className="text-zinc-200">{rows.length}</span> of{" "}
      ...
      {/* Desktop header */}
      <div className="hidden border-b border-zinc-800/70 pb-2 text-[11px] text-zinc-500 sm:grid sm:grid-cols-[minmax(0,2fr)_140px_110px] sm:gap-4">
        ...
        {loading && !rows.length ? (
          <>
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="flex animate-pulse flex-col rounded-2xl border border-zinc-900 bg-zinc-950/90 px-3 py-3 sm:grid sm:grid-cols-[minmax(0,2fr)_140px_110px] sm:items-center sm:gap-4"
              >
                ...
                <div className="mt-3 h-3 w-20 rounded bg-zinc-900 sm:mt-0 sm:justify-self-end" />
          ...
          rows.map((token) => {
            ...
            return (
              <Link
                key={token.mint}
                href={`/invest/${slug}`}
                className="group block rounded-2xl border border-zinc-900 bg-zinc-950/80 px-3 py-3 transition hover:border-emerald-500/50 hover:bg-zinc-950 sm:px-4 sm:py-3"
              >
                <div className="flex flex-col gap-3 sm:grid sm:grid-cols-[minmax(0,2fr)_140px_110px] sm:items-center sm:gap-4">
      ...
      {shouldShowPagination && (
        <div className="mt-4 flex items-center justify-between text-xs text-zinc-400">
          ...
          <div>
            Page <span className="font-semibold text-zinc-100">{page}</span> of{" "}
