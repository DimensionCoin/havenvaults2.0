...
export default function CryptoExchange() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const tokensRes = await fetch("/api/tokens?kind=crypto&pageSize=200", {
          method: "GET",
  ...
  const categoriesForTabs = useMemo(() => {
    ...
    for (const a of allAssets) {
      for (const c of a.categories ?? []) {
        if (c === "Stocks" || c === "PreMarket") continue;
        allCatsSet.add(c);
      }
    }
        ...
        <div className="mb-6">
          <div className="mb-2 flex items-end justify-between gap-4">
            <div>
              ...
              <div className="haven-title">Crypto</div>
              <div className="mt-1 text-sm text-muted-foreground">
                Buy, sell, and track tokens instantly.
              </div>
            </div>
          ...
          <div className="mt-4">
            <SearchBar
              value={q}
              onChange={setQ}
              placeholder="Search SOL, BTC, BONK…"
              hint="Search by name or symbol"
              assets={allAssets}
              limit={8}
              onSelectAsset={(a) => {
                // Clear so user immediately returns to the browse flow
                setQ("");
